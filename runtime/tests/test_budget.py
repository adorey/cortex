...
def test_spent_since_windows_by_started_at():
    ...
    _spend(store, "bluspark", 1.0, at=NOW - 10)            # inside 24h
    _spend(store, "bluspark", 2.0, at=NOW - DAY_S - 100)   # outside 24h
    _spend(store, "other", 5.0, at=NOW)                    # different workspace
    assert store.spent_since("bluspark", NOW - DAY_S) == pytest.approx(1.0)
    assert store.spent_since("bluspark", NOW - MONTH_S) == pytest.approx(3.0)
    assert store.spent_since("bluspark", NOW + 1) == 0.0
...
def test_no_tenant_or_no_ceiling_is_allowed():
    ...
    bare = TenantRecord("bluspark")   # no ceilings configured
...
def test_daily_cap_blocks_when_exceeded():
    store = InMemoryStateStore()
    _spend(store, "bluspark", 4.0, at=NOW - 100)
    _spend(store, "bluspark", 1.5, at=NOW - 100)
    tenant = TenantRecord("bluspark", budget_daily_usd=5.0)
...
def test_daily_cap_allows_under_ceiling_and_reports_remaining():
    store = InMemoryStateStore()
    _spend(store, "bluspark", 2.0, at=NOW - 100)
    tenant = TenantRecord("bluspark", budget_daily_usd=5.0)
    d = check_budget(store, tenant, now=NOW)
...
def test_old_spend_outside_window_does_not_count():
    store = InMemoryStateStore()
    _spend(store, "bluspark", 100.0, at=NOW - DAY_S - 1)   # yesterday+ → outside daily window
    tenant = TenantRecord("bluspark", budget_daily_usd=5.0)
...
def test_monthly_cap_independent_of_daily():
    ...
    for i in range(1, 6):
        _spend(store, "bluspark", 3.0, at=NOW - i * 2 * DAY_S)
    tenant = TenantRecord("bluspark", budget_daily_usd=5.0, budget_monthly_usd=10.0)
    d = check_budget(store, tenant, now=NOW)
