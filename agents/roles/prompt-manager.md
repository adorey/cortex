...
2. **Context aggregation (workspace mode only):** Unlike the layers below, workspace-level context is **additive, not cascaded** — there's no single winner to resolve ([ADR-006](../../docs/adr/ADR-006-workspace-shareable-repo.md)). Read both tiers when present, team first:
   - `{workspace_root}/agents/project-overview.md` / `project-context.md` — the **team** tier, if the team has versioned it
   - `{workspace_root}/project-overview.md` / `project-context.md` — the **developer** tier (this is the pre-existing, single-project-mode path — in workspace mode it's simply named as belonging to the individual developer)
   Keep each labeled by scope in the response context; don't silently blend them. In single-project mode (no workspace), this step is a no-op — there's only ever the one root context.
3. **Workflow lookup (cascade):** Search for a matching workflow for the detected context, walking the cascade:
