...
### Phase 3 — the shim runs the module as a file, isolated

§3.5 describes the shim as running `python3 -m cortex_core.validate`. Implemented that way, it put the working directory at the head of `sys.path`, ahead of the standard library: validating a project whose root holds, say, an `fnmatch.py` executed that file — code from the very project under validation, in CI included. The Bash implementation had no such exposure.

The shim therefore runs `core/cortex_core/validate.py` as a file under `python -I` — isolated mode: neither the working directory nor the script's directory lands on `sys.path`, and `PYTHON*` variables are ignored. The module consequently imports the standard library only; a test enforces it, and another runs the validator from a project root carrying hostile `fnmatch.py`, `re.py`, `typing.py` and `cortex_core/` files. Options, output and exit codes are unchanged.
