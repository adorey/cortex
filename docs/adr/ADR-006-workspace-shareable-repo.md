...
The runtime reads the team tier too since ADR-007 phase 4, first, as §3.3 orders. The tiers of `project-overview.md` and of `project-context.md` reach the agent's system prompt, under `# Project overview` and `# Project context`, with the workspace's services listed between them — what the Prompt Manager reads in the editor. The runtime labels them — `## Team context`, `## Developer notes`, `## Service context — {service}`, and the same for the overview — as soon as two or more exist: with a single tier there is no scope to tell apart, and it reads as it always did. The Prompt Manager's protocol of §3.3 is unchanged. See [ADR-007 §9](ADR-007-cortex-core.md#9-amendments).
